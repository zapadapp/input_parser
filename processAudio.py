...
sys.path.insert(0, os.path.join(WORKSPACE, "instrument_cnn"))
...
NOTE_TRANSLATION_MAP = {"A#2":"La#2","A#3":"La#3","A#4":"La#4","A#5":"La#5",
                        ...
                        "G2":"Sol2","G3":"Sol3","G4":"Sol4","G5":"Sol5","C6":"Do6","D6":"Re6"}

note_model = keras.models.load_model('../notes/modelo-notas-vfin.h5')
chord_model = keras.models.load_model('../chord_cnn/modelo-acordes-vte2.h5')
instrument_model = keras.models.load_model('../notes/modelo-instument-v0.h5')
...
def detectAndPrintNote(s, q, drawer, scorePath, y, sr, samples, a, b):
    ...
    if len(properties['peak_heights']) > 0:
        ...
        if len(peak_i) > 0:
            frec = f[peak_i[0]]
            nota = convertToNote(str(librosa.hz_to_note(frec)))
...
def processAudio(s, drawer, q, audioPath, scorePath,lastNote, lastDuration,detected,isFlute):
    ...
    if isSound(y,sr):
            ...
            for i in indexes[0]:
                ...
                if j < length-1:
                    ...
                    if duration > 0.20:
                        ...
                        if nota != "":
                            print("La duracion es:{}".format(duration))
                    ...
                    duration = onset_times[length-1]
                    if duration > 0.20:
                        if isFlute:
                            nota = detectAndPrintNote(s,q,drawer,scorePath, y, sr, filteredSamples, j, 999)
                        else :
                            _ , nota = getNoteAndInstrumentFromRNN(s,q,scorePath, y, sr, filteredSamples, j, 999)             # figure = getFigure(duration)
                    # drawer.drawNote(nota,figure)
        ...
        lastDuration = 0
        lastNote = ""
    return lastNote, lastDuration

def getInstrumentFromRNN(s, q, scorePath, data, sr, samples):
    instrument = ""
    ...
    if correctNoteShape(mel_spec.shape[1]):
        mel_reshape = tf.reshape(mel_spec, [ 1,N_MELS,SHAPE ])
        my_prediction = instrument_model.predict(mel_reshape)
        index = np.argmax(my_prediction)
...
def getChroma(signal,sample_rate):
    # extract Chroma
    chroma = librosa.feature.chroma_cens(y=signal, sr=sample_rate,fmin=80,n_octaves=4)
