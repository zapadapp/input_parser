...
#Without RNNOTES
def detectAndPrintNote(s, q, y, sr, samples, a, b):
...
def processAudio(s, drawer, q, audioPath, scorePath,lastNote, lastDuration,detected,noteDetected, flute):
    y, sr = librosa.load(audioPath)
    duration = 0.0
    nota = ""
    chord = ""
    soundBool = isSound(y,sr)
    if soundBool:
        ONSET_PARAM = 1
        ...
        if lastNote != "":
            if detected == "note":
                ...
                m21Chord = c21.Chord(getNotesFromChords(getBaseNote(lastNote),4))
        ...
        for i in indexes[0]:
            j = i
            if j < len(onset_times) - 1:
                duration = onset_times[j + 1] - onset_times[j]
    
            if duration < 0.15 and j != length-1:
                print("SKIPPINGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG")
                continue
            if detected == 'note':
                if j < length-1:
                    if flute:
                        nota = detectAndPrintNote(s,q, y, sr, filteredSamples, j, j+1)
                    else:    
                        _ , nota = getNoteAndInstrumentFromRNN(s,q,scorePath, y, sr, filteredSamples, j, j+1)
                    
                    if nota != 'not recognize' or nota != '':
                ...
                if j < length-1:
                    ...
                    if chord != '' :
                        ...
                        m21Chord.duration = d21.Duration(getFigureDuration(figure))
                        s.append(m21Chord)
                elif j == length-1:
...
def getNotesFromChords(chord,octava):
    ...
    harcode_octavas = ["F", "F#", "G", "G#", "A", "A#", "B", "B#"]
    if chord in harcode_octavas:
        octava = 3
    # octaveIndex = 2
    secondNoteIndex = 4
...
def filterLowSamples(samples):
    # find indexes of all elements lower than 500 from samples
    indexes = np.where(samples < 500)
